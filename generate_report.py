...
now = datetime.utcnow()
print("Calling Claude API for stock analysis...")
client = anthropic.Anthropic(api_key=os.environ["ANTHROPIC_API_KEY"])

prompt = f"""Analiza las siguientes acciones basándote en las noticias proporcionadas. Responde ÚNICAMENTE con un JSON válido, sin explicaciones ni markdown.

FECHA HOY: {now.strftime("%A %d de %B de %Y")} (en español)

NOTICIAS:
{json.dumps(all_news, indent=2, ensure_ascii=False)}

Devuelve un JSON con esta estructura exacta para las 11 acciones:
{{
  ...
  "acciones": [
    {{
      ...
      "razonamiento": "2-3 frases en español justificando la recomendación basada en las noticias y el análisis técnico.",
      "noticias": [
        {{
          ...
          "fecha": "17 abr 2026",
          "url": "https://...",
          "descripcion": "Una frase en español describiendo la noticia.",
          "oficial": false
        }}
      ]
    }}
...
Reglas:
- recomendacion debe ser exactamente: COMPRAR, MANTENER, VENDER o ESPECULATIVO
- oficial=true si la fuente es GlobeNewswire, PR Newswire, BusinessWire, o IR de la empresa
- rsi es un número entero entre 0 y 100
- Incluye las 12 acciones: TSLA, HIMS, DUOL, BMNR, LODE, HGRAF, HBFG, ABX, 3350.T, BILD, LIB, TTT
...
data = json.loads(raw)
print(f"  Got data for {len(data['acciones'])} stocks")
